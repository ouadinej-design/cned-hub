...
export default function SESPage() {
  useEffect(() => {
    ...
    const flush = () => {
      if (seconds > 0) {
        const today = new Date().toISOString().split("T")[0];
        supabase.from("time_log").insert({ event_date: today, matiere: "SE", seconds, ts: Date.now() }).then(() => {}, () => {});
        seconds = 0;
      }
    };
  ...
  return (
    <div style={{ background:"#0f172a", minHeight:"100vh", color:"#e2e8f0", fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      <div style={ padding:"0 16px 12px" }>
        <SessionTimer matiere="SES" />
        <RevisionFlash matiere="SES" matiereCode="SE" />
      </div>
    ...
    {exs.map((ex) => { const gi = s.exercises.indexOf(ex); const r = res[gi]; return (
      <div key={gi} style={{ background:"#1e293b", borderRadius:14, padding:18, marginBottom:12, borderLeft:`3px solid ${LEVELS[ex.level].c}` }}>
        ...
        <div style={{ display:"flex", gap:8, marginTop:10 }}>
          <button onClick={() => { const ok=checkAnswer(ans[gi]||"", ex.answer); setRes({...res,[gi]:ok}); setSessionAttempted(a=>a+1); if(ok) setSessionCorrect(c=>c+1); if(!ok) { logDifficulty("SES", s.title, ex.q, ans[gi]||"(vide)", ex.answer, ex.hint); fetch("/api/ai/corriger", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ type:"erreur_analyse", matiere:"SES", question:ex.q, reponse:ans[gi]||"(vide)", exercices:ex.answer }) }).then(r=>r.json()).then(d=>{ if(d.reply) setErrMsgs(m=>({...m,[gi]:d.reply})); }).catch(()=>{}); } supabase.from("attempts_log").insert({ event_date:new Date().toISOString().split("T")[0], matiere:"SE", seance:s.title, type:"exercice", identifier:ex.q, correct:ok, ts:Date.now() }).then(()=>{},()=>{}); }}
...
function Quiz({ s, mark }) {
  ...
  const nxt = () => { if(qi<s.quiz.length-1){ setQi(qi+1); setSel(null); setSh(false); } else { setFin(true); const finalScore = sc+(sel===q.correct?1:0); if(finalScore>=Math.ceil(s.quiz.length*.6)) mark(s.id,"quiz"); const wrong = s.quiz.filter((qq,idx) => idx<=qi && !(idx===qi?sel===qq.correct:true)).map(qq=>qq.q); logQuizResult("SES", s.title, finalScore, s.quiz.length, wrong);  supabase.from("attempts_log").insert({ event_date:new Date().toISOString().split("T")[0], matiere:"SE", seance:s.title, type:"quiz", identifier:s.title, correct:finalScore>=Math.ceil(s.quiz.length*.6), score:finalScore, total:s.quiz.length, ts:Date.now() }).then(()=>{},()=>{}); } };
...
function AiChat({ ctx, txt }) {
  const storageKey = "profIA_Ses_" + ctx;
  ...
  const ask = async () => {
    ...
    try {
      const r = await fetch("/api/ai/corriger", { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ type:"aide", question:u, reponse:u, section:ctx, cours:txt, matiere:"SES", history:newMsgs.slice(0,-1) }) });
      const d = await r.json();
  ...
  if(!open) return <div onClick={()=>setOpen(true)} style={{ background:"linear-gradient(135deg,#10b981,#34d399)", padding:14, borderRadius:12, textAlign:"center", cursor:"pointer", marginTop:16, fontWeight:700, fontSize:14, color:"#fff" }}>{msgs.length > 0 ? "🤖 Prof IA (conversation en cours)" : "🤖 Je suis bloqué — Prof IA"}</div>;
  return (<div style={{ background:"#1e293b", borderRadius:14, padding:18, marginTop:16, border:"2px solid #10b981" }}>
    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}><span style={{ fontWeight:700, color:"#34d399" }}>🤖 Prof de SES IA</span><div style={{ display:"flex", gap:10, alignItems:"center" }}>{msgs.length>0 && <span onClick={clearHistory} style={{ cursor:"pointer", fontSize:11, color:"#f87171" }}>Effacer</span>}<span onClick={()=>setOpen(false)} style={{ cursor:"pointer", color:"#94a3b8" }}>✕</span></div></div>
