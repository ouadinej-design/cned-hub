...
export default function HistGeoPage() {
  useEffect(() => {
    ...
    const flush = () => {
      if (seconds > 0) {
        const today = new Date().toISOString().split("T")[0];
        supabase.from("time_log").insert({ event_date: today, matiere: "HI", seconds, ts: Date.now() }).then(() => {}, () => {});
        seconds = 0;
      }
    };
  ...
  const demanderReexplication = async (gi, ex) => {
    ...
    try {
      const r = await fetch("/api/ai/corriger", { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ type:"reexplique", matiere:"Histoire-Géographie", section:s.title, cours:ex.q, question:ex.q, reponse:ans[gi]||"" }) });
...
function Quiz({ s, mark }) {
  ...
  const nxt = () => { if(qi<s.quiz.length-1){ setQi(qi+1); setSel(null); setSh(false); } else { setFin(true); const finalScore = sc+(sel===q.correct?1:0); if(finalScore>=Math.ceil(s.quiz.length*.6)) mark(s.id,"quiz"); const wrong = s.quiz.filter((qq,idx) => idx<=qi && !(idx===qi?sel===qq.correct:true)).map(qq=>qq.q); logQuizResult("Histoire-Géographie", s.title, finalScore, s.quiz.length, wrong);  supabase.from("attempts_log").insert({ event_date:new Date().toISOString().split("T")[0], matiere:"HI", seance:s.title, type:"quiz", identifier:s.title, correct:finalScore>=Math.ceil(s.quiz.length*.6), score:finalScore, total:s.quiz.length, ts:Date.now() }).then(()=>{},()=>{}); } };
...
function AiChat({ ctx, txt }) {
  const storageKey = "profIA_Histgeo_" + ctx;
  ...
  const ask = async () => {
    ...
    try {
      const r = await fetch("/api/ai/corriger", { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ type:"aide", question:u, reponse:u, section:ctx, cours:txt, matiere:"Histoire-Géographie", history:newMsgs.slice(0,-1) }) });
      const d = await r.json();
  ...
  if(!open) return <div onClick={()=>setOpen(true)} style={{ background:"linear-gradient(135deg,#f97316,#fb923c)", padding:14, borderRadius:12, textAlign:"center", cursor:"pointer", marginTop:16, fontWeight:700, fontSize:14, color:"#fff" }}>{msgs.length > 0 ? "🤖 Prof IA (conversation en cours)" : "🤖 Je suis bloqué — Prof IA"}</div>;
  return (<div style={{ background:"#1e293b", borderRadius:14, padding:18, marginTop:16, border:"2px solid #f97316" }}>
    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}><span style={{ fontWeight:700, color:"#fb923c" }}>🤖 Prof d'Hist-Géo IA</span><div style={{ display:"flex", gap:10, alignItems:"center" }}>{msgs.length>0 && <span onClick={clearHistory} style={{ cursor:"pointer", fontSize:11, color:"#f87171" }}>Effacer</span>}<span onClick={()=>setOpen(false)} style={{ cursor:"pointer", color:"#94a3b8" }}>✕</span></div></div>
