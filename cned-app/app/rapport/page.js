...
export default function RapportPage() {
  ...
  useEffect(() => {
    try { setActivity(JSON.parse(localStorage.getItem("activity_log") || "{}")); } catch {}
    try { setDifficulties(JSON.parse(localStorage.getItem("difficulties_log") || "{}")); } catch {}
  }, [offset]);

  const date = new Date(); date.setDate(date.getDate() - offset);
  const ds = isoDate(date);
  const dayActivity = activity[ds] || {};
  const dayDiff = difficulties[ds] || {};
  const matieresToday = [...new Set([...Object.keys(dayActivity), ...Object.keys(dayDiff)])];
    ...
    <div style={{ background: "#0f172a", minHeight: "100vh", color: "#e2e8f0", fontFamily: "'Inter',system-ui,sans-serif", padding: "16px 16px 80px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "#94a3b8" }}>Pour le professeur particulier</div>
        </div>
      </div>
        ...
        return (
          <div key={m} style={{ background: "#1e293b", borderRadius: 14, padding: 16, marginBottom: 12, borderLeft: `4px solid ${col}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              {dayActivity[m] && <span style={{ fontSize: 11, color: "#22c55e", fontWeight: 600 }}>✓ Travaillé</span>}
            </div>
            ...
            {quizzes.map((q, i) => (
              <div key={i} style={{ marginBottom: 10, padding: 10, background: "rgba(99,102,241,.08)", borderRadius: 8 }}>
                ...
                {q.wrongQuestions?.length > 0 && (
                  <div style={{ marginTop: 6 }}>
                    {q.wrongQuestions.map((w, wi) => <div key={wi} style={{ fontSize: 11, color: "#fca5a5" }}>✗ {w}</div>)}
                  </div>
                )}
              </div>
            ))}
              ...
              <div>
                ...
                {exos.map((e, i) => (
                  <div key={i} style={{ marginBottom: 8, padding: 10, background: "rgba(239,68,68,.08)", borderRadius: 8, fontSize: 12 }}>
                    ...
                    <div style={{ color: "#fca5a5" }}>Réponse donnée : "{e.userAnswer}"</div>
                    <div style={{ color: "#86efac" }}>Attendu : "{e.correctAnswer}"</div>
                  </div>
                ))}
              </div>
            )}
